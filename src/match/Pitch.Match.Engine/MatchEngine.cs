...
namespace Pitch.Match.Engine
{
    ...
    public class MatchEngine : IMatchEngine
    {
        ...
        private void SimulateMinute(Models.Match match, int minute)
        {
        ...
        private static void AssignStats(Models.Match match, int minute, Squad inPossession, int homePossChance, int awayPossChance)
        {
            match.Minutes[minute].Stats = new MinuteStats(inPossession.Id, homePossChance, awayPossChance);
        }

        private void ApplyModifiers(Models.Match match, int minute)
        {
            _fitnessDrainService.Drain(match, minute);
        }

        private void SetCalculatedStats(Models.Match match)
        {
